...
namespace osmscout {
                                                         ...
                                                         const double lookupDistance)
  {
    ...
    for (const auto& candidate : candidates) {
      for (size_t i = 0;  i < candidate->GetNodes().size() - 1; i++) {
        double r, intersectLon, intersectLat;
        double distance = DistanceToSegment(location.GetLon(),location.GetLat(),candidate->GetNodes()[i].GetLon(),candidate->GetNodes()[i].GetLat(),
                        candidate->GetNodes()[i+1].GetLon(),candidate->GetNodes()[i+1].GetLat(), r, intersectLon, intersectLat);
